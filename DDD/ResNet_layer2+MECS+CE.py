...
"""
Diabetic Retinopathy Detection Baseline using ResNet50 + MECS (Layer2) + CE
...
- MECS module inserted after layer2 (channels=512)
...
# =======================
# Custom Model: ResNet50 + MECS (Layer 2)
# =======================
class ResNet50_MECS_Layer2(nn.Module):
    def __init__(self, num_classes=5):
        super(ResNet50_MECS_Layer2, self).__init__()
        # 1. 加载预训练的 ResNet50
        ...
        # 3. 插入 MECS 模块 (ResNet50 layer2 输出通道数为 512)
        self.mecs = MECS_VersionA(in_channels=512, out_channels=512)
    ...
    def forward(self, x):
        ...
        # 在 layer2 和 layer3 之间通过 MECS 模块
        x = self.mecs(x)
...
# =======================
def main():
    print("Starting Diabetic Retinopathy ResNet50 + MECS (Layer 2) + CrossEntropy Baseline...")
    ...
    print("Loading pretrained ResNet50 and injecting MECS after layer2...")
    model = ResNet50_MECS_Layer2(num_classes=num_classes)
    ...
    # 更改保存路径，以便于区分这是带有 MECS + CE 的模型
    best_path = "best_resnet50_dr_mecs_layer2_ce_patient_split.pt"
    ...
    print("\nDiabetic Retinopathy ResNet50 + MECS (Layer 2) + CrossEntropy baseline completed!")
