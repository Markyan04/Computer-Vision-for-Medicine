...
"""
Diabetic Retinopathy Detection Baseline using ResNet50 + MDFA (After Layer3) + CrossEntropy
...
- MDFA module inserted after layer3 (channels=1024)
...
- Standard CrossEntropyLoss applied
...
# =======================
class EarlyStopping:
    def __init__(self, patience=10, delta=0.0, save_path="best_resnet50_dr_ce.pt"):
...
# =======================
# Custom Model: ResNet50 + MDFA (After Layer3)
# =======================
class ResNet50_MDFA_Layer3(nn.Module):
    def __init__(self, num_classes=5):
        super(ResNet50_MDFA_Layer3, self).__init__()
        # 1. 加载预训练的 ResNet50
        ...
        # 3. 插入 MDFA 模块 (ResNet50 layer3 输出通道数为 1024)
    ...
    def forward(self, x):
        ...
        # 在 layer3 和 layer4 之间通过 MDFA 模块
        x = self.mdfa(x)
...
# =======================
def main():
    print("Starting Diabetic Retinopathy ResNet50 + MDFA (Layer3) + CrossEntropy Baseline...")
    ...
    print("Loading pretrained ResNet50 and injecting MDFA after layer3...")
    model = ResNet50_MDFA_Layer3(num_classes=num_classes)
    ...
    # 使用普通交叉熵损失函数
    # =======================================================
    print(f"Initializing Standard CrossEntropyLoss for {num_classes} classes...")
    ...
    for name, param in model.named_parameters():
        # 将分类头 (fc) 和新初始化的 MDFA 模块 (mdfa) 都放入 head_params 中，使用较大的学习率
    ...
    # 统一命名风格，防止文件覆盖和混淆
    best_path = "best_resnet50_dr_mdfa_layer3_ce_patient_split.pt"
    ...
    print("\nDiabetic Retinopathy ResNet50 + MDFA (Layer3) + CrossEntropy baseline completed!")
