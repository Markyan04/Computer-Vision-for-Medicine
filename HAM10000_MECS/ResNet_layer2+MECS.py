...
"""
ISIC (HAM10000) Baseline using ResNet50 + MECS(after layer2)
...
class MECS(nn.Module):
    def __init__(self, in_channels, out_channels, channel_attention_reduce=4):
        ...
        # 【修复 BUG 1】：将原本复用的 self.pointwise_conv 拆分为三个独立权重的卷积层
...
class ResNet(nn.Module):
    def __init__(self, config, output_dim):
        ...
        # layer2 后接 MECS
        # layer2 的输出通道数是 block.expansion * channels[1] (对于 ResNet50 来说是 4 * 128 = 512)
        self.mecs = MECS(block.expansion * channels[1], block.expansion * channels[1])
...
# -----------------------
def main():
    print("Starting ISIC ResNet50 + MECS(layer2) ...")
    ...
    params = [
        ...
        {"params": model.mecs.parameters(), "lr": FOUND_LR / 4},  # 位置调整到 layer2 后，维持稍大的 lr
    ...
    # 保存路径更改为 layer2
    best_path = "isic_resnet50_mecs_layer2_best.pt"
    ...
    print("\nISIC ResNet50 + MECS(layer2) completed!")
