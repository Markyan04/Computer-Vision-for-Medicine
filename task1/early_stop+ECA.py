...
class ECAAttention(nn.Module):
    """
    ECA模块: 通过1D卷积高效捕获通道间依赖性，无需降维。
    来源: 组会文档《ECA-Net: Efficient Channel Attention for Deep Convolutional Neural Networks》
    """
    ...
    def forward(self, x):
        ...
        y = y.squeeze(-1).permute(0, 2, 1)  # 变为 bs, 1, c
        y = self.conv(y)  # 1D卷积
        y = self.sigmoid(y)  # 激活得到权重
        y = y.permute(0, 2, 1).unsqueeze(-1)  # 恢复维度 bs, c, 1, 1
        return x * y.expand_as(x)  # 特征重新校准
...
def main():
    ...
    best_path = "isic_resnet50_model_early_ECA.pt"
