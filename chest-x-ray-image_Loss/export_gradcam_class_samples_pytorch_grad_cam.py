...
"""Export Grad-CAM visualizations for every chest X-ray test image of a chosen class."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Export Grad-CAM visualizations for all chest X-ray test images of a chosen class.'
    )
    parser.add_argument('--class-name', required=True, help='True-label class name to filter, e.g. NORMAL, PNEUMONIA, COVID19.')
...
def main() -> None:
    ...
    module = load_script_module(script_path, prefix='chest_gradcam_class_pgc')
