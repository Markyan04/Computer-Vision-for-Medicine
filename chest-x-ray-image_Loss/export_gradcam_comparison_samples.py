...
"""Export chest X-ray test samples where baseline fails but proposed model succeeds, with Grad-CAM visualizations."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Export test-set Grad-CAM comparison samples for chest X-ray.',
    )
    parser.add_argument(
        '--class-name',
        help='True-label class name to filter, e.g. NORMAL, PNEUMONIA, COVID19.',
    )
    parser.add_argument(
        '--max-samples',
        type=int,
    ...
    parser.add_argument(
        '--image-size',
        type=int,
        default=int(os.getenv('CHESTXRAY_IMAGE_SIZE', '224')),
        help='Resize size. Default reads CHESTXRAY_IMAGE_SIZE or falls back to 224.',
    )
    parser.add_argument(
        '--alpha',
        type=float,
    ...
    parser.add_argument(
        '--data-root',
        default='',
        help='Optional chest X-ray data root override. Defaults to CHESTXRAY_DATA_ROOT or PROJECT_ROOT/CPN.',
    )
    parser.add_argument(
        '--test-dir',
        default='',
    ...
    parser.add_argument(
        '--list-classes',
        action='store_true',
        help='List available chest X-ray classes and exit.',
    )
...
def load_model(script_name: str, checkpoint_path: Path, num_classes: int, device: torch.device) -> torch.nn.Module:
    ...
    module = load_script_module(script_path, prefix='chest_gradcam_compare')
...
def main() -> None:
    ...
    if args.list_classes:
        print('Available chest X-ray classes:')
    ...
    if not args.class_name:
        ...
        print('Use --list-classes to inspect available chest X-ray labels.')
        return
