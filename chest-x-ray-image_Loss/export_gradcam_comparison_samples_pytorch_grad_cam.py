...
"""Export chest X-ray test samples where baseline fails but proposed succeeds, using pytorch-grad-cam."""
...
from gradcam_shared import extract_logits, load_script_module, sanitize_filename, tensor_to_pil  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Export chest X-ray test-set Grad-CAM comparison samples with pytorch-grad-cam.')
    parser.add_argument('--class-name', help='True-label class name to filter, e.g. NORMAL, PNEUMONIA, COVID19.')
...
def load_model(script_name: str, checkpoint_path: Path, num_classes: int, device: torch.device) -> torch.nn.Module:
    ...
    module = load_script_module(script_path, prefix='chest_gradcam_pgc_compare')
