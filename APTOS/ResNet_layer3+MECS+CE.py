...
"""
APTOS 2019 Blindness Detection baseline using ResNet50 + MECS (After Layer3) + CrossEntropy.
...
- Custom insertion of MECS_VersionA module after layer3 (Channels: 1024)
...
# =======================
# Custom Model with MECS (Layer 3)
# =======================
class CustomResNet50WithMECSLayer3(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        super(CustomResNet50WithMECSLayer3, self).__init__()
...
# =======================
class EarlyStopping:
    def __init__(self, patience=10, delta=0.0, save_path="best_aptos2019_resnet50_mecs_layer3_CE.pt"):
...
# =======================
def main():
    print("Starting APTOS2019 CustomResNet50+MECS (Layer3) Baseline...")
    ...
    print("Loading CustomResNet50 with MECS module (After Layer3)...")
    # 实例化在 Layer3 之后插入 MECS 的网络
    model = CustomResNet50WithMECSLayer3(num_classes=NUM_CLASSES, pretrained=True)
    ...
    for name, param in model.named_parameters():
        if "fc" in name:
            ...
            # MECS 模块的参数也会被算入 backbone 中，使用 backbone 的学习率
    ...
    # 保存的文件名做了修改，防止覆盖 layer2 的结果
    best_path = "best_aptos2019_resnet50_mecs_layer3_CE.pt"
    ...
    print("\nAPTOS2019 ResNet50 + MECS (Layer3) + CrossEntropy baseline completed!")
