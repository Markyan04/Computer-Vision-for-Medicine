...
"""
APTOS 2019 Blindness Detection baseline using ResNet50 + MDFA (After Layer3) + DAST.
Adapted from the Layer2 version.

Features:
- ResNet50 pretrained on ImageNet
- Custom insertion of MDFA module after layer3 (Channels: 1024)
...
# =======================
# Custom Model with MDFA (Layer 3)
# =======================
class CustomResNet50WithMDFALayer3(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        super(CustomResNet50WithMDFALayer3, self).__init__()
...
# =======================
class EarlyStopping:
    # 修改了默认的保存路径名，以区分 layer2 和 layer3
    def __init__(self, patience=10, delta=0.0, save_path="best_aptos2019_resnet50_mdfa_layer3_DAST.pt"):
...
# =======================
def main():
    print("Starting APTOS2019 CustomResNet50+MDFA (Layer3) Baseline...")
    ...
    print("Loading CustomResNet50 with MDFA module (After Layer3)...")
    # 实例化在 Layer3 之后插入 MDFA 的网络
    model = CustomResNet50WithMDFALayer3(num_classes=NUM_CLASSES, pretrained=True)
    model = model.to(DEVICE)

    print(f"Trainable parameters: {count_parameters(model):,}")

    # 使用 Distance-Aware Soft Target Loss
    ...
    best_path = "best_aptos2019_resnet50_mdfa_layer3_DAST.pt"
    ...
    print("\nAPTOS2019 ResNet50 + MDFA (Layer3) + DAST Loss baseline completed!")
