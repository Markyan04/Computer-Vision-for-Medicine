...
"""
APTOS 2019 Blindness Detection baseline using ResNet50 + GCSA (After Layer3) + CrossEntropy.
Adapted from the Layer2 baseline.
...
- Custom insertion of GCSA module after layer3 (Channels: 1024)
...
# =======================
# Custom Model with GCSA (Layer 3)
# =======================
class CustomResNet50WithGCSALayer3(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        super(CustomResNet50WithGCSALayer3, self).__init__()
        ...
        # --- 插入 GCSA 模块 ---
        # ResNet50 的 layer3 输出通道数为 1024
        self.gcsa = GCSA(in_channels=1024, rate=4)
...
# =======================
class EarlyStopping:
    # 更改了保存路径名，防止覆盖 layer2 的结果
    def __init__(self, patience=10, delta=0.0, save_path="best_aptos2019_resnet50_gcsa_layer3_CE.pt"):
...
# =======================
def main():
    print("Starting APTOS2019 CustomResNet50+GCSA (Layer3) Baseline...")
    ...
    print("Loading CustomResNet50 with GCSA module (After Layer3)...")
    # 实例化在 Layer3 之后插入 GCSA 的网络
    model = CustomResNet50WithGCSALayer3(num_classes=NUM_CLASSES, pretrained=True)
    model = model.to(DEVICE)

    print(f"Trainable parameters: {count_parameters(model):,}")

    criterion = nn.CrossEntropyLoss()
    print("\n[INFO] Using Standard CrossEntropyLoss (No class weighting)")
    ...
    for name, param in model.named_parameters():
        if "fc" in name:
            ...
            # GCSA 模块的参数也会被算入 backbone 中，使用 backbone 的学习率
    ...
    # 保存的文件名也做了修改，防止覆盖 layer2 的结果
    best_path = "best_aptos2019_resnet50_gcsa_layer3_CE.pt"
    ...
    print("\nAPTOS2019 ResNet50 + GCSA (Layer3) + CrossEntropy baseline completed!")
