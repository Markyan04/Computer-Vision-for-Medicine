...
"""
ISIC (HAM10000) Baseline using ResNet50 + MDFA
...
# -----------------------
class EarlyStopping:
    def __init__(self, patience=7, delta=0.0, save_path="isic_resnet50_mdfa_best_model.pt"):
        """
...
class ChannelAttention(nn.Module):
    """
    通道注意力：
    Global Avg Pool -> 1x1 Conv降维 -> ReLU -> 1x1 Conv升维 -> Sigmoid
    输出权重 shape = [B, C, 1, 1]
    """
...
class MDFA(nn.Module):
    ...
    接在 ResNet50 的 layer3 和 layer4 之间
    """
    ...
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1, reduction=2):
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(
                dim_in, dim_out,
                kernel_size=3, stride=1,
                padding=12 * rate, dilation=12 * rate,
                bias=False
            ),
            nn.BatchNorm2d(dim_out, momentum=bn_mom),
            nn.ReLU(inplace=True),
        ...
        self.branch4 = nn.Sequential(
            nn.Conv2d(
                dim_in, dim_out,
                kernel_size=3, stride=1,
                padding=18 * rate, dilation=18 * rate,
                bias=False
            ),
            nn.BatchNorm2d(dim_out, momentum=bn_mom),
            nn.ReLU(inplace=True),
...
# -----------------------
# ResNet (custom, baseline-compatible)
# -----------------------
...
# -----------------------
def main():
    print("Starting ISIC ResNet50 + MDFA...")
    ...
    # 只加载和当前模型名字、shape 都匹配的参数
    model_dict = model.state_dict()
    pretrained_dict = {
        k: v for k, v in tv_model.state_dict().items()
        if k in model_dict and v.shape == model_dict[k].shape
    }
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict, strict=False)

    print(f"Loaded pretrained params: {len(pretrained_dict)}/{len(model_dict)}")
    ...
    params = [
        ...
        {"params": model.mdfa.parameters(), "lr": FOUND_LR / 4},
        {"params": model.layer3.parameters(), "lr": FOUND_LR / 3},
    ...
    best_path = "isic_resnet50_mdfa_best_l2.pt"
