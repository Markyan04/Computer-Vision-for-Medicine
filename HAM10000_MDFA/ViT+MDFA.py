...
class TongDao(nn.Module):
    ...
    Output attention map only: [B, 1, H, W]
    """
    ...
    def forward(self, x):
        ...
        y = self.fc(y)  # [B, 1, 1, 1]
        y = self.relu(y)
        y = F.interpolate(y, size=(x.size(2), x.size(3)), mode='nearest')  # [B,1,H,W]
        return y
...
class HeBing(nn.Module):
    """
    Fuse channel attention and spatial attention
    Return attention map only
    """
    ...
    def forward(self, x):
        attn_channel = self.tongdao(x)  # [B,1,H,W]
        attn_spatial = self.kongjian(x)  # [B,1,H,W]
        attn = torch.max(attn_channel, attn_spatial)
        return attn
...
class MDFA(nn.Module):
    ...
    Bug fixed:
    - The old version returned already-weighted features from Hebing
    - Then multiplied feature_cat once again, causing double multiplication
    - Now Hebing returns attention map only, and gating is applied exactly once
    """
    ...
    def forward(self, x):
        ...
        global_feature = F.interpolate(global_feature, size=(row, col), mode='bilinear', align_corners=True)
        ...
        # 只做一次注意力加权
        attn = self.hebing(feature_cat)  # [B,1,H,W]
        feature_cat = feature_cat * attn  # single gating only
