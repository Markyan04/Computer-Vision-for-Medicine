...
- ClassBalancedFocalCELoss applied for highly imbalanced data
"""
...
# 导入 Class-Balanced Focal CE 损失函数
from medical_losses import ClassBalancedFocalCELoss
...
def compute_eval_metrics(y_true, y_pred, y_prob, num_classes, class_names):
    ...
    try:
        ...
        metrics["ovr_roc_auc_macro"] = roc_auc_score(y_true_onehot, y_prob, multi_class="ovr", average="macro")
        metrics["ovr_pr_auc_macro"] = average_precision_score(y_true_onehot, y_prob, average="macro")
    except Exception:
        pass
    return metrics
...
def main():
    ...
    train_idx, valid_idx = train_test_split(train_idx, test_size=0.1, stratify=train_targets, random_state=SEED)
    ...
    # ========================================================
    # 动态计算训练集的类别数量，并初始化 Class-Balanced Focal CE
    # ========================================================
    train_class_counts = np.bincount(train_targets, minlength=num_classes).tolist()
    print(f"Training class counts for CB-Focal loss: {train_class_counts}")
    print(f"Initializing ClassBalancedFocalCELoss for {num_classes} classes...")

    criterion = ClassBalancedFocalCELoss(
        class_counts=train_class_counts,
        beta=0.9999,  # 默认平滑因子
        gamma=2.0,  # Focal 聚焦因子
        smoothing=0.05  # 可选的一点 Label Smoothing
    ).to(device)
    # ========================================================
    ...
    TOPK = (1, 3)
    best_path = "../HAM10000_GCSA/isic_resnet50_gcsa_layer3_best.pt"
    early_stopping = EarlyStopping(patience=10, delta=0.0001, save_path=best_path)

    print("Starting training.")
    for epoch in range(EPOCHS):
        start_time = time.time()
        train_loss, train_top = train_one_epoch(model, train_loader, optimizer, criterion, scheduler, device, topk=TOPK)
        valid_loss, valid_top, valid_metrics = evaluate(model, valid_loader, criterion, device, num_classes,
                                                        class_names, topk=TOPK)
        end_time = time.time()
        ...
        print(
            f"  Valid | loss={valid_loss:.4f} | top1={valid_top['top1'] * 100:.2f}% | top3={valid_top['top3'] * 100:.2f}% | bal_acc={valid_metrics['balanced_acc'] * 100:.2f}% | macro_f1={valid_metrics['macro_f1']:.4f}")
    ...
    test_loss, test_top, test_metrics = evaluate(model, test_loader, criterion, device, num_classes, class_names,
                                                 topk=TOPK)

    print(
        f"\nTest | loss={test_loss:.4f} | top1={test_top['top1'] * 100:.2f}% | top3={test_top['top3'] * 100:.2f}% | acc={test_metrics['acc'] * 100:.2f}% | bal_acc={test_metrics['balanced_acc'] * 100:.2f}% | macro_f1={test_metrics['macro_f1']:.4f} | weighted_f1={test_metrics['weighted_f1']:.4f}")
