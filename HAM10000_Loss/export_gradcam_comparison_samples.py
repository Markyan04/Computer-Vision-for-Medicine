...
"""Export HAM10000 test samples where baseline fails but proposed model succeeds, with Grad-CAM visualizations."""
...
from ham10000_loss_experiment_common import SEED, sanitize_run_tag, set_seed, load_checkpoint_states  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Export test-set Grad-CAM comparison samples for HAM10000.',
    )
    parser.add_argument(
        '--class-name',
        help='True-label class name to filter, e.g. nv, mel, bkl.',
    )
    parser.add_argument(
        '--max-samples',
        type=int,
    ...
    parser.add_argument(
        '--list-classes',
        action='store_true',
        help='List available HAM10000 classes and exit.',
    )
...
def generate_overlay(
    ...
    original = tensor_to_pil(input_tensor.squeeze(0)).resize((image_size, image_size), resample=Image.Resampling.BILINEAR if hasattr(Image, 'Resampling') else Image.BILINEAR)
...
def collect_class_records(
    ...
    for row in split_records['test']:
        if str(row['dx']).strip().lower() == target:
            rows.append(row)
    return rows
...
def main() -> None:
    ...
    if not args.class_name:
        ...
        print('Use --list-classes to inspect available HAM10000 labels.')
        return
    ...
    for idx, row in enumerate(candidate_rows, start=1):
        ...
        original = tensor_to_pil(input_tensor.squeeze(0)).resize(
