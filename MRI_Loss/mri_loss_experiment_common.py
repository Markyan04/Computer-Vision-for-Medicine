...
"""Shared training and evaluation utilities for MRI loss experiments."""
...
def resolve_losses_to_run() -> List[str]:
    env_losses = os.getenv('MRI_LOSSES', '').strip()
    if not env_losses:
    ...
    if invalid:
        raise ValueError(f'Invalid loss names in MRI_LOSSES: {invalid}')
    return requested


def run_mri_medical_losses_experiments(
    script_stem: str,
    ...
    data_root = Path(os.getenv('MRI_DATA_ROOT', str(PROJECT_ROOT / 'MRI')))
    train_dir = Path(os.getenv('MRI_TRAIN_DIR', str(data_root / 'Training')))
    test_dir = Path(os.getenv('MRI_TEST_DIR', str(data_root / 'Testing')))

    val_ratio = float(os.getenv('MRI_VAL_RATIO', '0.15'))
    batch_size = int(os.getenv('MRI_BATCH_SIZE', '32'))
    epochs = int(os.getenv('MRI_EPOCHS', '50'))
    num_workers = int(os.getenv('MRI_NUM_WORKERS', '0'))
    image_size = int(os.getenv('MRI_IMAGE_SIZE', '224'))
    base_lr = float(os.getenv('MRI_BASE_LR', '1e-3'))
    patience = int(os.getenv('MRI_PATIENCE', '10'))
    early_delta = float(os.getenv('MRI_EARLY_DELTA', '1e-4'))
    loss_delta = float(os.getenv('MRI_LOSS_DELTA', '1e-4'))
    topk = DEFAULT_TOPK
