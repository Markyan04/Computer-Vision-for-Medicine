...
"""
Knee Osteoarthritis Severity Grading Baseline using ResNet(layer3) + MDFA + DAST Loss
...
# =======================
class EarlyStopping:
    def __init__(self, patience=10, delta=0.0, save_path="best_resnet50_mdfa_layer3_dast_knee_oa.pt"):
...
# =======================
# Custom Model: ResNet50 + MDFA (layer3)
# =======================
class CustomResNet50MDFA(nn.Module):
    def __init__(self, num_classes=5):
        super(CustomResNet50MDFA, self).__init__()
        ...
        # [MODIFIED]: MDFA Module (Output of layer3 has 1024 channels)
        self.mdfa = MDFA(dim_in=1024, dim_out=1024)
    ...
    def forward(self, x):
        ...
        # [MODIFIED]: Injecting MDFA after layer 3
        x = self.mdfa(x)
...
# =======================
def main():
    print("Starting ResNet(layer3)+MDFA+DAST Knee Osteoarthritis Baseline...")
    ...
    print("Loading ResNet50 with MDFA module at layer3...")
    model = CustomResNet50MDFA(num_classes=NUM_CLASSES)
    ...
    # ==========================================
    # 替换为 DistanceAwareSoftTargetLoss
    # ==========================================
    print("[INFO] Using Clinically Distance-Aware Soft Label Loss (DAST).")
    criterion = DistanceAwareSoftTargetLoss(num_classes=NUM_CLASSES, tau=1.0, gamma=1.5).to(device)

    backbone_params = []
    head_params = []
    for name, param in model.named_parameters():
        # 将新添加的 mdfa 层和全连接层 fc 一起放入 head_params 赋予更大的学习率
        if "fc" in name or "mdfa" in name:
    ...
    best_path = "best_resnet50_mdfa_layer3_dast_knee_oa.pt"
    ...
    print("\nResNet(layer3)+MDFA+DAST Knee Osteoarthritis baseline completed!")
