...
"""Shared helpers for pytorch-grad-cam visualizations."""
...
from gradcam_shared import BILINEAR, extract_logits, find_last_conv, resolve_target_root
...
def resolve_target_module(model: torch.nn.Module, target_layer: str, hint: Optional[str] = None) -> torch.nn.Module:
    target_root = resolve_target_root(model, target_layer)
    target_module = find_last_conv(target_root)
    if target_module is None:
        message = f'No Conv2d layer found under target-layer={target_layer}.'
        if hint:
            message += f' {hint}'
...
def predict(
    ...
    logits_extractor: Optional[Callable] = None,
) -> Tuple[int, float]:
    extractor = extract_logits if logits_extractor is None else logits_extractor
    with torch.no_grad():
        logits = extractor(model(input_tensor))
...
def build_cam_images(
    ...
    original = original_from_tensor(input_tensor.squeeze(0)).resize(
