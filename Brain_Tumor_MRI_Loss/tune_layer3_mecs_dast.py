...
"""Grid search DAST tau/gamma for Brain Tumor MRI ResNet layer3+MECS."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Tune DAST tau/gamma for Brain_Tumor_MRI ResNet layer3+MECS.',
    )
    parser.add_argument(
        '--taus',
        default='0.5,1.0,1.5',
    ...
    parser.add_argument(
        '--batch-size',
        type=int,
        default=int(os.getenv('BRAIN_MRI_BATCH_SIZE', '32')),
        help='Training batch size. Default reads BRAIN_MRI_BATCH_SIZE or falls back to 32.',
    )
    parser.add_argument(
        '--epochs',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Optional override for BRAIN_MRI_PATIENCE.',
    )
    parser.add_argument(
        '--early-delta',
        type=float,
        default=None,
        help='Optional override for BRAIN_MRI_EARLY_DELTA.',
    )
    parser.add_argument(
        '--loss-delta',
        type=float,
        default=None,
        help='Optional override for BRAIN_MRI_LOSS_DELTA.',
    )
    parser.add_argument(
        '--val-ratio',
        type=float,
        default=None,
        help='Optional override for BRAIN_MRI_VAL_RATIO.',
    )
    parser.add_argument(
        '--data-root',
        default='',
...
def build_env(args: argparse.Namespace, tau: float, gamma: float, run_tag: str) -> Dict[str, str]:
    ...
    env['BRAIN_MRI_DAST_TAU'] = str(tau)
    env['BRAIN_MRI_DAST_GAMMA'] = str(gamma)
    env['BRAIN_MRI_RUN_TAG'] = run_tag
    ...
    if args.patience is not None:
        env['BRAIN_MRI_PATIENCE'] = str(args.patience)
    if args.early_delta is not None:
        env['BRAIN_MRI_EARLY_DELTA'] = str(args.early_delta)
    if args.loss_delta is not None:
        env['BRAIN_MRI_LOSS_DELTA'] = str(args.loss_delta)
    if args.val_ratio is not None:
        env['BRAIN_MRI_VAL_RATIO'] = str(args.val_ratio)
    if args.data_root:
