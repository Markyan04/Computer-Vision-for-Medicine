...
"""Evaluate a Brain Tumor MRI checkpoint on the test split."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Evaluate a Brain Tumor MRI checkpoint on the test split.')
    ...
    parser.add_argument('--batch-size', type=int, default=int(os.getenv('BRAIN_MRI_BATCH_SIZE', '16')))
    parser.add_argument('--num-workers', type=int, default=int(os.getenv('BRAIN_MRI_NUM_WORKERS', '0')))
    parser.add_argument('--image-size', type=int, default=int(os.getenv('BRAIN_MRI_IMAGE_SIZE', '224')))
    parser.add_argument('--val-ratio', type=float, default=float(os.getenv('BRAIN_MRI_VAL_RATIO', '0.10')))
    parser.add_argument('--data-root', default='', help='Optional Brain MRI data directory override. Defaults to BRAIN_MRI_DATA_ROOT or PROJECT_ROOT/Brain_Tumor_MRI.')
...
def main() -> None:
    ...
    if loss_name == 'dast':
        if dast_tau is not None:
            os.environ['BRAIN_MRI_DAST_TAU'] = str(dast_tau)
        if dast_gamma is not None:
            os.environ['BRAIN_MRI_DAST_GAMMA'] = str(dast_gamma)

    data_root = Path(args.data_root or os.getenv('BRAIN_MRI_DATA_ROOT', str(PROJECT_ROOT / 'Brain_Tumor_MRI'))).resolve()
    train_dir = Path(args.train_dir or os.getenv('BRAIN_MRI_TRAIN_DIR', str(data_root / 'Training'))).resolve()
    test_dir = Path(args.test_dir or os.getenv('BRAIN_MRI_TEST_DIR', str(data_root / 'Testing'))).resolve()
    ...
    if noise_stds:
        ...
        for noise_std in noise_stds:
            ...
            qwk_text = f"{test_metrics['qwk']:.4f}" if test_metrics['qwk'] is not None else 'n/a'
            summary_line = (
                ...
                f"qwk={qwk_text} | mae={test_metrics['mae']:.4f}"
            )
            print(summary_line)
            log_lines.extend([
                f'[std={noise_std:.4f}]',
